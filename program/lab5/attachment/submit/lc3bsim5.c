...
int GetLD_WRITE(int *x)      { return(x[LD_WRITE]); }
int GetXMUX(int *x)          { return(x[XMUX]); }
int GetYMUX(int *x)          { return(x[YMUX]); }
int GetWRITEMUX(int *x)      { return(x[WRITEMUX]); }
int GetGATE_LOGIC1(int *x)   { return(x[GATE_LOGIC1]); }
int GetGATE_LOGIC2(int *x)   { return(x[GATE_LOGIC2]); }
int GetLD_VA(int *x)         { return(x[LD_VA]); }
int GetLD_RETURN(int *x)     { return(x[LD_RETURN]); }
int GetMODEMUX(int *x)       { return(x[MODEMUX]); }
int GetBEMUX(int *x)         { return(x[BEMUX]); }
int GetRYMUX(int *x)         { return(x[RYMUX]); }
int GetSETMDR0(int *x)       { return(x[SETMDR0]); }
...
void cycle() {
  ...
  CYCLE_COUNT++;
  if ((CYCLE_COUNT>=300)&&(CYCLE_COUNT<350))
    INT=1;
  if ((CYCLE_COUNT<300)||(CYCLE_COUNT>=350))
    INT=0;
}
...
void get_signal_val()
{
    ...
    my_ld_write = GetLD_WRITE(CURRENT_LATCHES.MICROINSTRUCTION);
    my_xmux = GetXMUX(CURRENT_LATCHES.MICROINSTRUCTION);
    my_ymux = GetYMUX(CURRENT_LATCHES.MICROINSTRUCTION);
    my_writemux = GetWRITEMUX(CURRENT_LATCHES.MICROINSTRUCTION);
    my_gate_logic1 = GetGATE_LOGIC1(CURRENT_LATCHES.MICROINSTRUCTION);
    my_gate_logic2 = GetGATE_LOGIC2(CURRENT_LATCHES.MICROINSTRUCTION);
    my_ld_va = GetLD_VA(CURRENT_LATCHES.MICROINSTRUCTION);
    my_ld_return = GetLD_RETURN(CURRENT_LATCHES.MICROINSTRUCTION);
    my_modemux = GetMODEMUX(CURRENT_LATCHES.MICROINSTRUCTION);
    my_bemux = GetBEMUX(CURRENT_LATCHES.MICROINSTRUCTION);
    my_rymux = GetRYMUX(CURRENT_LATCHES.MICROINSTRUCTION);
    my_setmdr0 = GetSETMDR0(CURRENT_LATCHES.MICROINSTRUCTION);
...
/*    if (my_rw==1 && my_data_size==1 && (CURRENT_LATCHES.MAR&0x0001)==1) {printf("store error!");}*/
    ...
    my_vectormux_in0=((CURRENT_LATCHES.MDR>>3)&0x0001)?2:4;
    switch (my_vectormux)
    {
        case 0:{my_vectormux_out=my_vectormux_in0;break;}
        case 1:{my_vectormux_out=1;break;}
        case 2:{my_vectormux_out=3;break;}
        case 3:{my_vectormux_out=5;break;}
    ...
    my_x_in=my_xmux?1:0;
    my_y_in=my_ymux?(BUS&0x0000):0;
    my_write_in=my_writemux?1:0;
    my_excbr_in=((CURRENT_LATCHES.MDR>>2)&0x0001) && ((CURRENT_LATCHES.MDR>>3)&0x0001);

    gate_logic1_in=CURRENT_LATCHES.PTBR+(((CURRENT_LATCHES.MAR>>9)&0x007f)<<1);
    gate_logic2_in=(CURRENT_LATCHES.MDR&0x3e00)+(CURRENT_LATCHES.VA&0x01ff);

    my_return_in=CURRENT_LATCHES.X?(0x0030+my_ir15_12_inv):37;
    my_bemux_out=my_bemux?CURRENT_LATCHES.EXCBR : CURRENT_LATCHES.BEN;
    my_rymux_out=my_rymux?CURRENT_LATCHES.Y : CURRENT_LATCHES.READY;
    my_modemux_out=my_modemux?CURRENT_LATCHES.PSR15 : ((CURRENT_LATCHES.IR >> 11) & 0x0001);
}
...
void eval_micro_sequencer()
{
   ...
   */
    ...
    if (my_ird)
        NEXT_LATCHES.STATE_NUMBER = my_myird?CURRENT_LATCHES.RETURN:(my_ir15_12);
    else
        switch (my_cond)
        {
        case 0:
        ...
        case 1:
            NEXT_LATCHES.STATE_NUMBER = my_j + (my_rymux_out << 1);break;
        case 2:
            NEXT_LATCHES.STATE_NUMBER = my_j + (my_bemux_out << 2);break;
        case 3:
            NEXT_LATCHES.STATE_NUMBER = my_j + my_modemux_out;break;
        case 4:
...
void drive_bus() {
   ...
   */
    ...
    if (my_gate_pc)         BUS=Low16bits(CURRENT_LATCHES.PC);
    if (my_gate_mdr)        BUS=Low16bits(logic2_mux2_out);
    if (my_gate_alu)        BUS=Low16bits(my_alu_out);
    if (my_gate_marmux)     BUS=Low16bits(my_marmux_out);
    if (my_gate_shf)        BUS=Low16bits(my_shf_out);
    if (my_gate_psr15)      BUS=BUS+((CURRENT_LATCHES.PSR15&0x0001)<<15);
    if (my_gate_cc)         BUS=BUS+((CURRENT_LATCHES.N&0x0001)<<2)+((CURRENT_LATCHES.Z&0x0001)<<1)+(CURRENT_LATCHES.P&0x0001);
    if (my_gate_pc2)        BUS=Low16bits(CURRENT_LATCHES.PC-2);
    if (my_gate_r6)         BUS=Low16bits(my_r6mux_out);

    if (my_gate_vector)     BUS=Low16bits(GATE_VECTOR_in);
    if (my_gate_logic1)     BUS=Low16bits(gate_logic1_in);
    if (my_gate_logic2)     BUS=Low16bits(gate_logic2_in);
...
void latch_datapath_values() {
   ...
   */
    ...
    */
    ...
    if(my_ld_write)     NEXT_LATCHES.WRITE=my_write_in;
    ...
    if(my_ld_va)        NEXT_LATCHES.VA=CURRENT_LATCHES.MAR;
    if(my_ld_return)    NEXT_LATCHES.RETURN=my_return_in;

    if(my_setmdr0 && (CURRENT_LATCHES.WRITE==1))      NEXT_LATCHES.MDR=((CURRENT_LATCHES.MDR&0xfffc)+3);
    if(my_setmdr0 && (CURRENT_LATCHES.WRITE==0))      NEXT_LATCHES.MDR=((CURRENT_LATCHES.MDR&0xfffe)+1);
