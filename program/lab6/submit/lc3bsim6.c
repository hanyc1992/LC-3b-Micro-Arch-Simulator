...
void MEM_stage() {
  ...
  /* ********************************************dcache logic***************************************************** */
  if (Get_DATA_SIZE(PS.MEM_CS)) write_word=PS.MEM_ALU_RESULT;
  else                          write_word=((PS.MEM_ALU_RESULT&0x00ff)<<8)+(PS.MEM_ALU_RESULT&0x00ff);

  v_dcache_en=PS.MEM_V & Get_DCACHE_EN(PS.MEM_CS);

  if(Get_DCACHE_RW(PS.MEM_CS)){
    if(Get_DATA_SIZE(PS.MEM_CS)){we1=1;we0=1;}
    else if(PS.MEM_ADDRESS&0x0001){we1=1;we0=0;}
    else {we1=0;we0=1;}
  }
  else {we1=0;we0=0;}

  if(v_dcache_en)   dcache_access(PS.MEM_ADDRESS, &read_word, write_word, &dcache_r, we0, we1);
  else              read_word=0;
  /* ********************************************************************************************************** */

  logic2_mux1_out=(PS.MEM_ADDRESS&0x0001)?((read_word>>8)&0x00ff):(read_word&0x00ff);
  sr_data_in=Low16bits((Get_DATA_SIZE(PS.MEM_CS)) ? (read_word) : (sext(logic2_mux1_out, 8)));

  /* *******************************************get signal for other stage******************************************* */
  if(PS.MEM_V){
    mem_pcmux=0;

    if(Get_UNCOND_OP(PS.MEM_CS))    mem_pcmux=1;
    if(Get_TRAP_OP(PS.MEM_CS))      mem_pcmux=2;
    if(Get_BR_OP(PS.MEM_CS)){
        if(((PS.MEM_IR>>9)&0x0007) & (PS.MEM_CC&0x0007))    mem_pcmux=1;
        else                                                mem_pcmux=0;
    }
  }
  else mem_pcmux=0;
  ...
  sr_v_in=PS.MEM_V & (!mem_stall);/* ******************************************************************************************think carefully*********************** */
...
void AGEX_stage() {
  ...
  /* ***************************************************internal signal eval******************************************* */

  int ir_4_0= (PS.AGEX_IR & 0x001f);

  if(Get_SR2MUX(PS.AGEX_CS))
    alu_b_in=Low16bits(sext(ir_4_0,5));
  else
    alu_b_in=PS.AGEX_SR2;


  switch (Get_ALUK(PS.AGEX_CS))
  {
  case 0:
    {alu_out = Low16bits(alu_b_in + PS.AGEX_SR1) ;
    break;}
  case 1:
    {alu_out = Low16bits(alu_b_in & PS.AGEX_SR1) ;
    break;}
  ...
  switch((PS.AGEX_IR>>4)&0x0003)
    {
        case 0:{shf_out=Low16bits(PS.AGEX_SR1<<(PS.AGEX_IR&0x000f));break;}/*lshf*/
        case 1:{shf_out=Low16bits(PS.AGEX_SR1>>(PS.AGEX_IR&0x000f));break;}/*rshfl*/
        case 3:{shf_out=Low16bits(sext(PS.AGEX_SR1,16)>>(PS.AGEX_IR&0x000f));break;}/*rshfa*/
        default:{break;}
    }

  mem_alu_result_in=(Get_ALU_RESULTMUX(PS.AGEX_CS)) ? (alu_out) : (shf_out);
  ...
  switch (Get_ADDR2MUX(PS.AGEX_CS))
    {
        ...
        case 1:{addr2mux_out=Low16bits(sext(PS.AGEX_IR&0x003f,6));break;}
        case 2:{addr2mux_out=Low16bits(sext(PS.AGEX_IR&0x01ff,9));break;}
        case 3:{addr2mux_out=Low16bits(sext(PS.AGEX_IR&0x07ff,11));break;}
        default:{printf("addr2mux error!");break;}
    }
  addressmux_in1=(addr1mux_out)+(addr2mux_out<<Get_LSHF1(PS.AGEX_CS));
  addressmux_in0=(PS.AGEX_IR & 0x00ff)<<1;
  addressmux_out=(Get_ADDRESSMUX(PS.AGEX_CS)) ? (addressmux_in1) : (addressmux_in0);
  /* ****************************************************************************************************************** */

  /* *************************************************************************************************generate ld_mem & mem_v_in*********************** */

  if(mem_stall) LD_MEM=0;
  else          LD_MEM=1;

  if(PS.AGEX_V) mem_v_in=1;
  else          mem_v_in=0;
...
void DE_stage() {
  ...
  /* ***************************************************internal signal eval******************************************* */
  CONTROL_STORE_ADDRESS=((PS.DE_IR>>10)&0x003e)+((PS.DE_IR>>5)&0x0001);
  my_br_stall=Get_DE_BR_STALL(CONTROL_STORE[CONTROL_STORE_ADDRESS]);
  my_sr1_needed=Get_SR1_NEEDED(CONTROL_STORE[CONTROL_STORE_ADDRESS]);
  my_sr2_needed=Get_SR2_NEEDED(CONTROL_STORE[CONTROL_STORE_ADDRESS]);
  my_drmux=Get_DRMUX(CONTROL_STORE[CONTROL_STORE_ADDRESS]);
  my_br_op=Get_DE_BR_OP(CONTROL_STORE[CONTROL_STORE_ADDRESS]);

  my_sr2_idmux=( ((PS.DE_IR&0xf000)==0x3000) || ((PS.DE_IR&0xf000)==0x7000) )?1:0;/* ************only for stb/stw************************ */
  my_sr2=my_sr2_idmux?((PS.DE_IR>>9)&0x0007):(PS.DE_IR&0x0007);
  my_sr1=((PS.DE_IR>>6)&0x0007);
  my_agex_sr1_in=REGS[my_sr1];
  my_agex_sr2_in=REGS[my_sr2];
  my_agex_cc_in=(N<<2)+(Z<<1)+P;
  my_agex_drid_in=(my_drmux) ? (7) : ((PS.DE_IR>>9)&0x0007);
  /* ****************************************************************************************************************** */

  /* *******************************************get signal for other stage******************************************* */
  v_de_br_stall=my_br_stall & PS.DE_V;
  ...
  if(PS.DE_V){
    if(my_br_op){
        ...
        if(v_agex_ld_reg && my_sr1_needed && (agex_reg_id==my_sr1)) dep_stall=1;
        else if(v_agex_ld_reg && my_sr2_needed && (agex_reg_id==my_sr2)) dep_stall=1;
        else if(v_mem_ld_reg && my_sr1_needed && (mem_reg_id==my_sr1)) dep_stall=1;
        else if(v_mem_ld_reg && my_sr2_needed && (mem_reg_id==my_sr2)) dep_stall=1;
        else if(v_sr_ld_reg && my_sr1_needed && (sr_reg_id==my_sr1)) dep_stall=1;
        else if(v_sr_ld_reg && my_sr2_needed && (sr_reg_id==my_sr2)) dep_stall=1;
        else dep_stall=0;
    }
  }
  else
  ...
  /* *************************************************************************************************generate ld_agex & my_agex_v_in*********************** */

  if(mem_stall) LD_AGEX=0;
  else          LD_AGEX=1;

  if(!PS.DE_V)          my_agex_v_in=0;
  else if(dep_stall)    my_agex_v_in=0;
  else                  my_agex_v_in=1;
  ...
  if(v_sr_ld_cc)    {N=sr_n;Z=sr_z;P=sr_p;}
  /* ****************************************************************************************************************** */
  ...
  if (LD_AGEX) {
    ...
    NEW_PS.AGEX_SR1=my_agex_sr1_in;
    NEW_PS.AGEX_SR2=my_agex_sr2_in;
    NEW_PS.AGEX_CC=my_agex_cc_in;
    NEW_PS.AGEX_DRID=my_agex_drid_in;
    NEW_PS.AGEX_V=my_agex_v_in;
...
void FETCH_stage() {
  ...
  my_mem_pcmux_in0=PC+2;
  switch (mem_pcmux)
  {
  case 0:
    my_pc_in = Low16bits(my_mem_pcmux_in0) ;
    break;
  case 1:
    my_pc_in = Low16bits(target_pc) ;
    break;
  case 2:
    my_pc_in = Low16bits(trap_pc) ;
    break;
  case 3:
  ...
  /* ******************************************************************generate ld_pc & ld_de & de_v_in*********************** */
  ld_pc=0;
  if(!v_de_br_stall && !v_agex_br_stall && !dep_stall && icache_r && !mem_stall && !v_mem_br_stall)    ld_pc=1;
  if(!mem_stall && v_mem_br_stall && mem_pcmux)  ld_pc=1;

  ld_de=0;
  if(!mem_stall && !dep_stall)      ld_de=1;

  de_v_in=0;
  if(!mem_stall && !dep_stall && !v_de_br_stall && !v_agex_br_stall && !v_mem_br_stall && icache_r) de_v_in=1;

  /* *********************************************************************************************************************************************** */

  if(ld_pc) PC=my_pc_in;

  if(ld_de){
    NEW_PS.DE_NPC=my_mem_pcmux_in0;
